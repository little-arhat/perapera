...
```
$FLUENT_DATA_DIR/results/{skill}-session-{ID}.md
```
...
- **Read `$FLUENT_DATA_DIR/results/` markdown for context.** Use `read-db.py` for numerical summaries — don't reimplement counts by re-parsing markdown when the DB already has them.
...
- **Never alter `$FLUENT_DATA_DIR/results/` files.** They are immutable records. Planning only.
