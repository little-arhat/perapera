...
Also save the exchange as `$FLUENT_DATA_DIR/results/fluent-writing-session-{NNN}.md` with the full task, the learner's original text, the corrected version, and the error table. The `fluent-session-analyzer` skill depends on this format.
...
- **Always save the session file** in `$FLUENT_DATA_DIR/results/` for later analysis by `fluent-session-analyzer`.
